...
| **Comic reader** for multi-page works: vertical strip or page by page right to left, with zoom; recommended manga on Home, manga rankings and series with every chapter | **Manga in your language**: the 訳 button on any picture (detail, reader or viewer) reads the page on the phone and works out by itself which language it is in (Japanese, Chinese, Korean or English); each line is translated into the app's language, Spanish or English, with the original underneath, like Danbooru's notes. By default it translates online with Google Translate, nothing to download; fetch the dictionaries in Settings and it translates offline | **Wallpaper that changes by itself**: every half hour, every few hours or once a day, with random works, the artists you follow, your bookmarks or lists you build from any work's menu; home, lock screen or both, Wi-Fi only if you want |
| **Read offline**: "Save to read offline" in any work's menu fetches every page with a progress bar at the bottom and a notification; saved works live in Bookmarks › Downloads and open the same way without a connection | | **New version notice**: once a day it looks at GitHub releases and tells you on Home and in Settings, with a direct APK download |
...
- Manga text recognition runs on the phone with Google's [ML Kit](https://developers.google.com/ml-kit); translation uses Google Translate's web service by default (only the recognised text is sent, never the picture) and, once the dictionaries are downloaded, ML Kit's offline models.
